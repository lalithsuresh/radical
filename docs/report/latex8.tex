...
The messages are generally quite large. While measuring the size of an empty Message object, i.e without any attributes such as destination, source, type, or payload attached to it, it determined to be 853 bytes big. As we can see, the message overhead is not signficantly impacted by our message data but rather by .NET's method for binary serialization. Binary serialization is used over tcp channels in .NET remoting. Consequently, this tells us that optimising the content of the message would yield less improvements compared to focusing on minimising the total number of messages. For example, usernames to uri lookups are done one and one. As an improvement lookups could be batched together to reduce messages. Further, clients do not cache any lookups, and therefore a new lookup is performed for every send. Even deffered sends. This also explains the relatively high number of messages counted in test case 7 even though Client 2 only was down for a few seconds. 

Furthermore, as noted in the description of test case 5, each message sent to another client requires a lookup message to be sent to the server. It would be possible to implement a lookup cache to minimize the number of lookup messages. That was, however, not considered a primary task of this project.
